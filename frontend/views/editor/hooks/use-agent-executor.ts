  ...
  const handleGetTimelineState = useCallback((): ToolResult => {
    ...
    return ok("get_timeline_state", {
      ...
      tracks: tracks.map((t, i) => ({
        ...
        muted: t.muted, locked: t.locked, solo: t.solo ?? false,
        enabled: t.enabled ?? true, sourcePatched: t.sourcePatched ?? true,
        type: t.type ?? "default",
      })),
      ...
      clips: clips.map((c) => ({
        ...
        trimEnd: c.trimEnd, speed: c.speed, reversed: c.reversed,
        muted: c.muted, volume: c.volume, prompt: c.asset?.prompt ?? null,
        linkedClipIds: c.linkedClipIds ?? [],
        ...
        effects: c.effects ?? [],
        hasTextOverlay: Boolean(c.textStyle),
