...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  getResourcePath: (): Promise<string | null> =>
    ipcRenderer.invoke("get-resource-path"),
  ...
  checkPythonReady: (): Promise<{ ready: boolean }> =>
    ipcRenderer.invoke("check-python-ready"),
  startPythonSetup: (): Promise<void> =>
    ipcRenderer.invoke("start-python-setup"),
  startPythonBackend: (): Promise<void> =>
    ipcRenderer.invoke("start-python-backend"),
  onPythonSetupProgress: (cb: (data: unknown) => void) => {
    ipcRenderer.on("python-setup-progress", (_: unknown, data: unknown) =>
      cb(data),
    );
  },
