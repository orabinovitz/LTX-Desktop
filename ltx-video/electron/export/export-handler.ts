    ...
    async (
      ...
      for (const seg of segments) {
        if (seg.filePath && !fs.existsSync(seg.filePath)) {
          return {
            error: `Source file not found: ${path.basename(seg.filePath)}`,
          };
        }
      }
      ...
      try {
        ...
        console.log(
          `[Export] Step 1: Video-only export (${segments.length} segments)`,
        );
        {
        ...
        let totalDuration = segments.reduce(
        ...
        const { pcmBuffer, sampleRate, channels } = await mixAudioToPcm(
          clips,
          totalDuration,
          ffmpegPath,
        );
        ...
        console.log(
          `[Export] Wrote raw PCM: ${pcmBuffer.length} bytes (${totalDuration.toFixed(2)}s)`,
        );
