  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) resolve(Buffer.concat(chunks));
        ...
        reject(
          new Error(`PCM extraction failed (code ${code}) for ${filePath}`),
        );
    });
...
export async function mixAudioToPcm(
  ...
  console.log(
    `[Export] Audio: ${audioSources.length} source(s) from ${clips.length} clip(s)`,
  );
  ...
  for (let i = 0; i < audioSources.length; i++) {
    ...
    console.log(
      `[Export] Audio ${i + 1}/${audioSources.length}: ${path.basename(src.filePath)} trim=${src.trimStart.toFixed(2)}-${src.trimEnd.toFixed(2)} @${src.timelineStart.toFixed(2)}s vol=${src.volume}`,
    );
    try {
      const pcm = await extractPcmBuffer(
        ffmpegPath,
        src.filePath,
        src.trimStart,
        src.trimEnd,
      ...
      console.log(
        `[Export] Audio ${i + 1}: mixed ${numPcmSamples} samples (${(numPcmSamples / SAMPLE_RATE / NUM_CHANNELS).toFixed(2)}s) at offset frame ${startFrame}`,
      );
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      console.warn(
        `[Export] Failed to extract audio from ${src.filePath}: ${message}`,
      );
    }
  }
