...
export function buildVideoFilterGraph(
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (seg.type === "gap") {
      ...
      inputs.push(
        ...
        "lavfi",
        "-i",
        `color=c=black:s=${width}x${height}:r=${fps}:d=${seg.duration.toFixed(6)}`,
      );
  ...
  filterParts.push(
    `${concatInputs}concat=n=${segments.length}:v=1:a=0[concatraw]`,
  );
  ...
  if (letterbox) {
    ...
    const alphaHex = Math.round(letterbox.opacity * 255)
      .toString(16)
      .padStart(2, "0");
    ...
    if (targetRatio >= containerRatio) {
      ...
      if (barH > 0) {
        filterParts.push(
          `[${lastLabel}]drawbox=x=0:y=0:w=iw:h=${barH}:c=${colorStr}:t=fill,drawbox=x=0:y=ih-${barH}:w=iw:h=${barH}:c=${colorStr}:t=fill[${nextLabel}]`,
        );
        lastLabel = nextLabel;
      }
    } else {
      ...
      if (barW > 0) {
        filterParts.push(
          `[${lastLabel}]drawbox=x=0:y=0:w=${barW}:h=ih:c=${colorStr}:t=fill,drawbox=x=iw-${barW}:y=0:w=${barW}:h=ih:c=${colorStr}:t=fill[${nextLabel}]`,
        );
        lastLabel = nextLabel;
      }
    }
  ...
  if (subtitles && subtitles.length > 0) {
    for (let si = 0; si < subtitles.length; si++) {
      ...
      if (
        ...
        const bgColor =
          bgHex.length > 6 ? `0x${bgHex.slice(0, 6)}` : `0x${bgHex}`;
        const bgAlpha =
          bgHex.length > 6
            ? (parseInt(bgHex.slice(6), 16) / 255).toFixed(2)
            : "0.6";
