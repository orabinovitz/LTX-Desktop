...
export function flattenTimeline(clips: ExportClip[]): FlatSegment[] {
  ...
  for (const seg of segments) {
    ...
    if (
      prev &&
      prev.filePath === seg.filePath &&
      prev.filePath !== "" &&
      prev.speed === seg.speed &&
      prev.reversed === seg.reversed &&
      prev.flipH === seg.flipH &&
      prev.flipV === seg.flipV &&
      prev.opacity === seg.opacity &&
      prev.muted === seg.muted &&
      prev.volume === seg.volume &&
      Math.abs(prev.trimStart + prev.duration * prev.speed - seg.trimStart) <
        0.01
    ) {
