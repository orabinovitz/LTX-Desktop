...
export function initAutoUpdater(channel: UpdateChannel = "latest"): void {
  ...
  autoUpdater.on("update-downloaded", async (info: UpdateDownloadedEvent) => {
    ...
    console.log(
      `[updater] Update downloaded: v${newVersion}, checking python deps...`,
    );

    try {
      const didDownload = await preDownloadPythonForUpdate(
        newVersion,
        (progress) => {
      ...
      if (didDownload) {
        console.log(
          "[updater] Python pre-download complete, installing update...",
        );
      } else {
      ...
      console.error(
        "[updater] Python pre-download failed, proceeding with update:",
        err,
      );
    }
