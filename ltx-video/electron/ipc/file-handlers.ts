...
function readLocalFileAsBase64(filePath: string): {
  data: string;
  mimeType: string;
} {
...
function searchDirectoryForFiles(
  dir: string,
  filenames: string[],
...
export function registerFileHandlers(): void {
  ...
  ipcMain.handle(
    "save-file",
    async (_event, filePath: string, data: string, encoding?: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, dir: string, filenames: string[]) => {
  ...
  ipcMain.handle(
    ...
    async (_event, sourcePath: string, originalName: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const mainWindow = getMainWindow();
      if (!mainWindow) return null;
      const props: Electron.OpenDialogOptions["properties"] = ["openFile"];
