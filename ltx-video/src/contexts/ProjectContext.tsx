...
interface ProjectContextType {
  ...
  updateProject: (
    id: string,
    updates: Partial<Pick<Project, "assetSavePath">>,
  ) => void;
  ...
  updateTimeline: (
    projectId: string,
    timelineId: string,
    updates: Partial<Pick<Timeline, "tracks" | "clips" | "subtitles">>,
  ) => void;
...
function recoverAssetUrls(project: Project): Project {
  ...
  const fixedTimelines = project.timelines?.map((tl) => ({
    ...
    clips:
      tl.clips?.map((clip) => {
        if (
          clip.asset?.url?.startsWith("blob:") &&
          isRealPath(clip.asset.path)
        ) {
          return {
            ...clip,
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const [genSpaceEditImageUrl, setGenSpaceEditImageUrl] = useState<
    string | null
  >(null);
  const [genSpaceEditMode, setGenSpaceEditMode] = useState<
    "image" | "video" | null
  >(null);
  ...
  const createProject = useCallback(
    (name: string, assetSavePath?: string): Project => {
  ...
  const updateProject = useCallback(
    (id: string, updates: Partial<Pick<Project, "assetSavePath">>) => {
      setProjects((prev) =>
        prev.map((p) =>
  ...
  const addAsset = useCallback(
    (projectId: string, assetData: Omit<Asset, "id" | "createdAt">): Asset => {
  ...
  const addTakeToAsset = useCallback(
    (projectId: string, assetId: string, take: AssetTake) => {
      setProjects((prev) =>
        prev.map((p) => {
  ...
  const deleteTakeFromAsset = useCallback(
    (projectId: string, assetId: string, takeIndex: number) => {
      setProjects((prev) =>
        prev.map((p) => {
          if (p.id !== projectId) return p;
          return {
            ...
            assets: p.assets.map((a) => {
              ...
              if (newActiveIdx >= newTakes.length)
                newActiveIdx = newTakes.length - 1;
  ...
  const setAssetActiveTake = useCallback(
    (projectId: string, assetId: string, takeIndex: number) => {
      setProjects((prev) =>
        prev.map((p) => {
          if (p.id !== projectId) return p;
  ...
  const addTimeline = useCallback(
    (projectId: string, name?: string): Timeline => {
  ...
  const deleteTimeline = useCallback(
    (projectId: string, timelineId: string) => {
      setProjects((prev) =>
        prev.map((p) => {
          if (p.id !== projectId) return p;
          const remaining = (p.timelines || []).filter(
            (t) => t.id !== timelineId,
          );
  ...
  const renameTimeline = useCallback(
    (projectId: string, timelineId: string, name: string) => {
      setProjects((prev) =>
        prev.map((p) =>
          p.id === projectId
            ? {
  ...
  const duplicateTimeline = useCallback(
    (projectId: string, timelineId: string): Timeline | null => {
      const project = projects.find((p) => p.id === projectId);
  ...
  const getActiveTimeline = useCallback(
    (projectId: string): Timeline | null => {
      const project = projects.find((p) => p.id === projectId);
      if (!project || !project.timelines || project.timelines.length === 0)
        return null;

      // Find the active timeline, or fall back to the first one
      const active = project.timelines.find(
        (t) => t.id === project.activeTimelineId,
      );
