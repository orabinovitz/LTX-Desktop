...
export function KeyboardShortcutsProvider({
  ...
  const [activePresetId, setActivePresetId] = useState<string>(
    () => loadFromStorage()?.activePresetId || "ltx-default",
  );
  const [customLayout, setCustomLayout] = useState<KeyboardLayout | null>(
    () => loadFromStorage()?.customLayout || null,
  );
  const [customPresets, setCustomPresets] = useState<KeyboardPreset[]>(
    () => loadFromStorage()?.customPresets || [],
  );
  ...
  const updateBinding = useCallback(
    (actionId: ActionId, combos: KeyboardLayout[ActionId]) => {
