...
const CODEC_INFO: Record<
  ExportCodec,
  { label: string; ext: string; description: string; filterName: string }
> = {
  h264: {
    label: "H.264 / MP4",
    ext: "mp4",
...
function generateFCPXML(
  ...
  for (const trackIdx of sortedTrackIndices) {
    ...
    for (const clip of trackClips) {
      ...
      const name =
        clip.asset?.prompt?.slice(0, 60) || clip.importedName || "Clip";
...
export function ExportModal({
  ...
  const [exportType, setExportType] = useState<"package" | "video" | null>(
    null,
  );
  ...
  const exportLetterbox = useMemo(() => {
    ...
    const ratio =
      lb.aspectRatio === "custom"
        ? lb.customRatio || 2.35
        : ratioMap[lb.aspectRatio] || 2.35;
    return {
      ratio,
      color: lb.color || "#000000",
      opacity: (lb.opacity ?? 100) / 100,
    };
  }, [clips, tracks]);
  ...
  const handleExportVideo = useCallback(async () => {
    ...
    try {
      ...
      const exportClips = clips
        .filter(
          (c) => c.type === "video" || c.type === "image" || c.type === "audio",
        )
      ...
      const subtitleData =
        burnSubtitles && timeline.subtitles
  ...
  return (
    ...
    >
      <div
        className="relative flex max-h-[calc(100vh-2rem)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-zinc-700/50 bg-zinc-900 shadow-2xl"
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="max-w-[340px] truncate text-xs text-zinc-500">
                    {exportPath}
                  </p>
                  {exportFrameInfo && (
                    <p className="text-xs text-zinc-500">{exportFrameInfo}</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="max-w-[340px] break-words text-xs text-red-400">
                    {exportError}
                  </p>
                </div>
              </div>
          ...
          {exportStatus === "idle" && (
            <div className="space-y-5">
              ...
              <button
                ...
                className="group flex w-full items-center gap-3 rounded-xl border border-zinc-700/50 bg-zinc-800/50 p-3 transition-all hover:border-zinc-600 hover:bg-zinc-800"
              >
                ...
                <div className="flex flex-shrink-0 items-center gap-1.5">
                  <div
                    className="flex h-6 w-6 items-center justify-center rounded bg-zinc-700"
                  ...
                  <Download className="ml-1 h-4 w-4 text-zinc-500 transition-colors group-hover:text-zinc-300" />
                </div>
              </button>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full cursor-pointer appearance-none rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 pr-8 text-sm text-white focus:border-blue-500 focus:outline-none"
                    >
                      {RESOLUTIONS.map((r) => (
                        <option
                  ...
                  <div className="grid grid-cols-4 gap-1.5">
                    {PRORES_PROFILES.map((p) => (
                      <button
                        key={p.value}
                        onClick={() =>
                          setSettings((prev) => ({ ...prev, quality: p.value }))
                        }
                ...
                <div className="space-y-2">
                  ...
                  <label className="group flex cursor-pointer items-center gap-2.5">
                    <input
                      ...
                      className="h-4 w-4 cursor-pointer rounded border-zinc-600 bg-zinc-800 accent-blue-500"
                    />
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-40"
              >
