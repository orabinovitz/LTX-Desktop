...
export function SettingsModal({
  ...
  const [textEncoderStatus, setTextEncoderStatus] =
    useState<TextEncoderStatus | null>(null);
  ...
  const handleDownloadTextEncoder = async () => {
    ...
    try {
      const backendUrl = await window.electronAPI.getBackendUrl();
      const response = await fetch(`${backendUrl}/api/text-encoder/download`, {
        method: "POST",
      });
      const data = await response.json();

      if (data.status === "already_downloaded") {
        setTextEncoderStatus((prev) =>
  ...
  const handleFastUpscalerToggle = () => {
    onSettingsChange({
      ...
      fastModel: {
        ...settings.fastModel,
        useUpscaler: !settings.fastModel?.useUpscaler,
      },
    });
  };
  ...
  const handleProUpscalerToggle = () => {
    onSettingsChange({
      ...
      proModel: {
        ...settings.proModel,
        useUpscaler: !settings.proModel.useUpscaler,
      },
    });
  };
  ...
  const handleTogglePromptEnhancer = (mode: "t2v" | "i2v") => {
    if (mode === "t2v") {
      onSettingsChange({
        ...settings,
        promptEnhancerEnabledT2V: !settings.promptEnhancerEnabledT2V,
      });
    } else {
      onSettingsChange({
        ...settings,
        promptEnhancerEnabledI2V: !settings.promptEnhancerEnabledI2V,
      });
    }
  };
  const anyEnhancerEnabled =
    settings.promptEnhancerEnabledT2V || settings.promptEnhancerEnabledI2V;
  ...
  const handleT2vSystemPromptChange = (
  ...
  const handleI2vSystemPromptChange = (
    e: React.ChangeEvent<HTMLTextAreaElement>,
  ) => {
    onSettingsChange({
      ...settings,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative mx-4 w-full max-w-lg rounded-xl border border-zinc-700 bg-zinc-900 shadow-2xl">
        {/* Header */}
            ...
            <>
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs leading-relaxed text-zinc-500">
                  Text encoding converts your prompt into data the AI
                  understands. Choose how to do this.
                </p>

                {/* LTX API Option (Default) */}
                <div
                  className={`cursor-pointer rounded-lg border-2 bg-zinc-800/50 p-4 transition-colors ${
                  ...
                  {!settings.useLocalTextEncoder && (
                    <div className="mt-3 border-t border-zinc-700/50 pt-3">
                      <input
                        ...
                        className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white placeholder-zinc-500 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                      ...
                      {settings.ltxApiKey && (
                        <div className="mt-3 flex items-center justify-between border-t border-zinc-700/50 pt-3">
                          ...
                          <input
                            ...
                            onClick={(e) => e.stopPropagation()}
                            className="w-16 rounded border border-zinc-600 bg-zinc-700 px-2 py-1 text-center text-xs text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                          />
                        </div>
                      )}
                    </div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-xs text-zinc-400">
                        Run on your computer (~23 seconds). Requires 8 GB
                        download.
                      </p>
                    </div>
                    <div
                    ...
                    <div className="mt-3 border-t border-zinc-700/50 pt-3">
                      {textEncoderStatus?.downloaded ? (
                        <div className="flex items-center gap-2 text-xs text-green-400">
                          ...
                          <span>
                            Downloaded ({textEncoderStatus.size_gb} GB)
                          </span>
                        </div>
                      ) : isDownloading ? (
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-xs text-amber-400">
                            ...
                            <span>
                              Not downloaded (
                              {textEncoderStatus?.expected_size_gb || 8} GB
                              required)
                            </span>
                          </div>
                          <Button
                            ...
                            className="w-full bg-blue-600 text-xs text-white hover:bg-blue-500"
                          >
              ...
              <div className="space-y-3 border-t border-zinc-800 pt-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs leading-relaxed text-zinc-500">
                      Load AI models in the background after the app starts. The
                      video model is loaded and warmed up on GPU, and the image
                      model is preloaded into CPU RAM for faster first
                      generation. When disabled, models load on first use
                      (faster startup, slower first generation). Requires app
                ...
                >
                  ...
                  />
                  {settings.loadOnStartup
                    ? "Models preload in background at startup"
                    : "Models load on first generation"}
                </div>
              </div>
              ...
              <div className="space-y-3 border-t border-zinc-800 pt-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs leading-relaxed text-zinc-500">
                      ...
                      <span className="text-orange-400">Experimental:</span>{" "}
                      First generation can take 5-10+ minutes for compilation.
                      Subsequent generations may be 20-40% faster. Requires app
                      restart to take effect.
                    </p>
                  </div>
                ...
                >
                  ...
                  />
                  {settings.useTorchCompile
                    ? "Optimized inference (recommended)"
                    : "Standard inference"}
                </div>
              </div>
              ...
              <div className="space-y-3 border-t border-zinc-800 pt-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs leading-relaxed text-zinc-500">
                      Use the same seed for reproducible generations. When
                      unlocked, a random seed is used each time.
                    </p>
                  </div>
                ...
                {settings.seedLocked && (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-white focus:border-transparent focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    ...
                    <Button
                      ...
                      className="h-9 px-3 text-xs text-zinc-400 hover:bg-zinc-800 hover:text-white"
                ...
                >
                  ...
                  />
                  {settings.seedLocked
                    ? `Seed locked: ${settings.lockedSeed ?? 42}`
            ...
            <>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 rounded-lg bg-zinc-800/50 p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">
                        Fixed at 8 steps (built into distilled model)
                      </p>
                    </div>
                    <span className="rounded-lg bg-zinc-700 px-3 py-1.5 text-sm text-zinc-400">
                      8
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">
                        When off, generates at native resolution
                      </p>
                    </div>
                    <button
                      onClick={handleFastUpscalerToggle}
                      className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                        settings.fastModel?.useUpscaler !== false
                          ? "bg-green-500"
                          : "bg-zinc-700"
                      }`}
                    >
                ...
                <div className="text-xs text-zinc-500">
                  Current: 8 steps,{" "}
                  {settings.fastModel?.useUpscaler !== false
                    ? "with upscaler (2-stage, recommended)"
                    : "native resolution (experimental)"}
                </div>
              </div>
              ...
              <div className="space-y-4 border-t border-zinc-800 pt-4">
                ...
                <div className="space-y-4 rounded-lg bg-zinc-800/50 p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      className="w-20 rounded-lg border border-zinc-600 bg-zinc-700 px-3 py-1.5 text-center text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <label className="text-sm text-white">2x Upscaler</label>
                      <p className="text-xs text-zinc-500">
                        Doubles resolution in second pass
                      </p>
                    </div>
                    <button
                ...
                {/* Summary */}
                <div className="text-xs text-zinc-500">
                  Current: {settings.proModel?.steps ?? 20} steps,{" "}
                  {settings.proModel?.useUpscaler !== false
                    ? "with upscaler (2-stage, recommended)"
                    : "native resolution"}
                </div>
              ...
              <div className="mt-4 rounded-lg bg-zinc-800/30 p-3">
                <p className="text-xs text-zinc-400">
                  <span className="font-medium text-blue-400">Tip:</span> Lower
                  steps = faster but lower quality. Higher steps = better
                  quality but slower.
                </p>
              </div>
            </>
          ...
          {activeTab === "promptEnhancer" && (
            <>
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs leading-relaxed text-zinc-500">
                  Automatically enhances your prompts with rich visual details,
                  sound descriptions, and motion cues to help the AI generate
                ...
                {/* T2V Toggle */}
                <div
                  className="flex cursor-pointer items-center justify-between rounded-lg border border-zinc-700/50 bg-zinc-800/50 px-4 py-3"
                  onClick={() => handleTogglePromptEnhancer("t2v")}
                >
                  <div className="flex items-center gap-3">
                    <span className="rounded bg-blue-400/10 px-1.5 py-0.5 text-xs font-semibold text-blue-400">
                      T2V
                    </span>
                    <div>
                      ...
                      <p className="mt-0.5 text-[10px] text-zinc-500">
                        {settings.promptEnhancerEnabledT2V
                          ? "Prompts will be enhanced before T2V generation"
                          : "T2V prompts used as-is"}
                      </p>
                    </div>
                  </div>
                  <div
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-[10px] text-zinc-500">
                        {settings.promptEnhancerEnabledI2V
                          ? "Prompts will be enhanced before I2V generation"
                          : "I2V prompts used as-is"}
                      </p>
                    </div>
                  </div>
                  <div
              ...
              {anyEnhancerEnabled && (
                <div className="space-y-3 border-t border-zinc-800 pt-4">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-white placeholder-zinc-500 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
              ...
              {settings.promptEnhancerEnabledT2V && (
                <div className="space-y-3 border-t border-zinc-800 pt-4">
                  ...
                  <textarea
                    value={
                      settings.t2vSystemPrompt || DEFAULT_T2V_SYSTEM_PROMPT
                    }
                    onChange={handleT2vSystemPromptChange}
                    rows={8}
                    className="w-full resize-none rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 font-mono text-xs text-white placeholder-zinc-500 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
              ...
              {settings.promptEnhancerEnabledI2V && (
                <div className="space-y-3 border-t border-zinc-800 pt-4">
                  ...
                  <textarea
                    value={
                      settings.i2vSystemPrompt || DEFAULT_I2V_SYSTEM_PROMPT
                    }
              ...
              {anyEnhancerEnabled && (
                <div className="mt-2 rounded-lg bg-zinc-800/30 p-3">
                  <p className="text-xs text-zinc-500">
                    <span className="text-zinc-400">Note:</span> Prompt
                    enhancement is not applied to text-to-image generation.
                  </p>
                </div>
              )}
              ...
              {showResetConfirm && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center">
                  ...
                  />
                  <div className="relative mx-4 w-full max-w-sm rounded-xl border border-zinc-700 bg-zinc-900 p-6 shadow-2xl">
                    <h3 className="mb-2 text-lg font-semibold text-white">
                      Reset{" "}
                      {showResetConfirm === "t2v"
                        ? "Text-to-Video"
                        : "Image-to-Video"}{" "}
                      Prompt?
                    </h3>
                    <p className="mb-4 text-sm text-zinc-400">
                      This will restore the default system prompt. Any changes
                    ...
                    <div className="flex justify-end gap-3">
                      ...
                      <Button
                        onClick={() =>
                          handleResetSystemPrompt(showResetConfirm)
                        }
