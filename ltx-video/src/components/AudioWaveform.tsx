...
export const waveformCache = new Map<string, Float32Array>(); // eslint-disable-line react-refresh/only-export-components
...
export function AudioWaveform({
  ...
  const audioClipUrlsKey = audioClips.map((c) => c.url).join(",");
  useEffect(() => {
    let cancelled = false;
    const loadAll = async () => {
  ...
  return (
    <div ref={containerRef} className="flex h-full w-full flex-col">
      ...
      {/* Clip names */}
      {audioClips.length > 0 && (
        <div className="flex-shrink-0 border-t border-zinc-800 bg-zinc-950 px-3 py-1.5">
          {audioClips.map((clip, i) => (
            <p key={i} className="truncate text-[10px] text-zinc-500">
              {clip.name}
            </p>
          ))}
        </div>
