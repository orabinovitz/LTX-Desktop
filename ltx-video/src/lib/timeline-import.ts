...
function parseFcp7ClipEffects(clipEl: Element): ParsedClipEffects {
  ...
  effects.forEach((effect) => {
    ...
    // ── Speed / Time Remap ──
    if (
      effectId.includes("timeremap") ||
      effectId.includes("speed") ||
      effectName.includes("time remap") ||
      effectName.includes("speed")
    ) {
      // Get all parameters
    ...
    // ── Horizontal Flip ──
    if (
      effectId.includes("horizflip") ||
      effectId.includes("hflip") ||
      effectName.includes("horizontal flip") ||
      effectName.includes("flip horizontal")
    ) {
      result.flipH = true;
    }
    ...
    // ── Vertical Flip ──
    if (
      effectId.includes("vertflip") ||
      effectId.includes("vflip") ||
      effectName.includes("vertical flip") ||
      effectName.includes("flip vertical")
    ) {
      result.flipV = true;
    }
    ...
    if (
      effectId === "basic" ||
      effectName.includes("basic motion") ||
      effectName.includes("motion")
    ) {
      const params = effect.querySelectorAll(":scope > parameter");
      params.forEach((param) => {
        const paramId = getChildText(param, "parameterid").toLowerCase();
  ...
  const compositeOpacity = clipEl.querySelector(
    ":scope > compositemode > opacity > value",
  );
  if (compositeOpacity) {
...
function parseFcp7Xml(doc: Document): ParsedTimeline | null {
  ...
  const sequence =
    doc.querySelector("xmeml > sequence") ||
  ...
  const seqFormat = sequence.querySelector(
    ":scope > media > video > format > samplecharacteristics",
  );
  ...
  fileElements.forEach((fileEl) => {
    ...
    const fileName =
      getChildText(fileEl, "name") || pathUrl.split("/").pop() || "Unknown";
    const fileFps = getTimebase(fileEl);
    const fileDuration = getChildNumber(fileEl, "duration") / (fileFps || fps);

    // Get media dimensions
    const mediaVideo = fileEl.querySelector(
      ":scope > media > video > samplecharacteristics",
    );
    const width = mediaVideo ? getChildNumber(mediaVideo, "width") : undefined;
    const height = mediaVideo
      ? getChildNumber(mediaVideo, "height")
      : undefined;
  ...
  // Video tracks
  const videoTracks = sequence.querySelectorAll(
    ":scope > media > video > track",
  );
  ...
  // Audio tracks
  const audioTracks = sequence.querySelectorAll(
    ":scope > media > audio > track",
  );
  audioTracks.forEach((trackEl, audioIdx) => {
    ...
    clipItems.forEach((clipEl) => {
      ...
      const volFilter = Array.from(
        clipEl.querySelectorAll("filter > effect"),
      ).find(
        (e) =>
      ...
      const linkedVideoIndex = clips.findIndex(
        (c) =>
      ...
      if (startFrame >= 0 && endFrame > startFrame && mediaRefId) {
        clips.push({
          ...
          volume,
          muted: effects.muted || undefined,
          linkedVideoClipIndex:
            linkedVideoIndex >= 0 ? linkedVideoIndex : undefined,
        });
      }
    });
  });
  ...
  for (const group of audioGroups.values()) {
    ...
    if (representative.linkedVideoClipIndex === undefined) {
      ...
      if (linked)
        representative.linkedVideoClipIndex = linked.linkedVideoClipIndex;
    }
  ...
  const finalAudioTrackCount =
    mergedAudioTrackIdx || (audioTrackCount > 0 ? 1 : 0);
  ...
  return {
    ...
    duration:
      totalDuration ||
      Math.max(...finalClips.map((c) => c.startTime + c.duration), 0),
    width: seqWidth,
    height: seqHeight,
    videoTrackCount,
...
function parseFcpxmlClipEffects(el: Element): Partial<ParsedClipEffects> {
  ...
  if (timeMap) {
    ...
    if (timepts.length >= 2) {
      ...
      const srcDur =
        parseFcpxmlTimeLocal(last.getAttribute("time")) -
        parseFcpxmlTimeLocal(first.getAttribute("time"));
      const dstDur =
        parseFcpxmlTimeLocal(last.getAttribute("value")) -
        parseFcpxmlTimeLocal(first.getAttribute("value"));
  ...
  filterVideos.forEach((fv) => {
    ...
    if (
      filterName.includes("flipped") ||
      filterName.includes("flip") ||
...
function parseFcpXml(doc: Document): ParsedTimeline | null {
  ...
  const project =
    fcpxml.querySelector("project") || fcpxml.querySelector("event > project");
  const sequence =
    project?.querySelector("sequence") || fcpxml.querySelector("sequence");

  if (!sequence) return null;

  const name =
    sequence.getAttribute("name") ||
  ...
  assets.forEach((assetEl) => {
    ...
    if (assetFormat) {
      const fmtEl = fcpxml.querySelector(
        `resources > format[id="${assetFormat}"]`,
      );
      if (fmtEl) {
    ...
    const type =
      !hasVideo && hasAudio ? "audio" : detectMediaType(assetName || src);
  ...
  if (spine) {
    ...
    for (let i = 0; i < spineChildren.length; i++) {
      ...
      if (
        ...
        const clipOffset =
          parseFcpxmlTime(el.getAttribute("offset")) || currentOffset;
        ...
        if (ref && mediaRefs.has(ref)) {
          ...
          clips.push({
            ...
            opacity:
              fx.opacity !== undefined && fx.opacity !== 100
                ? fx.opacity
                : undefined,
        ...
        const attached = el.querySelectorAll(
          ":scope > asset-clip, :scope > clip, :scope > audio, :scope > video",
        );
        attached.forEach((att) => {
          ...
          if (attRef && mediaRefs.has(attRef)) {
            ...
            clips.push({
              ...
              reversed:
                attFx.speed !== undefined && attFx.speed < 0 ? true : undefined,
              flipH: attFx.flipH || undefined,
              flipV: attFx.flipV || undefined,
              opacity:
                attFx.opacity !== undefined && attFx.opacity !== 100
                  ? attFx.opacity
                  : undefined,
        ...
        attached.forEach((att) => {
          ...
          if (attRef && mediaRefs.has(attRef)) {
            ...
            clips.push({
              ...
              trackIndex: attTrackIdx,
              trackType:
                att.tagName.toLowerCase() === "audio" ? "audio" : "video",
  ...
  const fcpxFinalAudioCount =
    fcpxMergedTrackIdx || (audioTrackCount > 0 ? 1 : 0);
  ...
  return {
    ...
    duration:
      totalDuration ||
      Math.max(...fcpxFinalClips.map((c) => c.startTime + c.duration), 0),
    width: seqWidth,
    height: seqHeight,
    videoTrackCount,
...
export function detectFormat(content: string, filename: string): ImportFormat {
  ...
  // Check XML content
  if (content.includes("<xmeml") || content.includes("<!DOCTYPE xmeml"))
    return "fcp7xml";
...
export function parseTimelineXml(
  content: string,
  filename: string,
): ParsedTimeline | null {
  ...
  if (parseError) {
    throw new Error(
      "Invalid XML file: " +
        (parseError.textContent?.slice(0, 200) || "Parse error"),
    );
  }
  ...
  throw new Error(
    "Could not parse timeline. The file format is not recognized as FCP 7 XML or FCPXML.",
  );
}
