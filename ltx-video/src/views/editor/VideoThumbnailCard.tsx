    ...
    >
      {thumbnailUrl ? (
        <img
          src={thumbnailUrl}
          alt=""
          className={`absolute inset-0 h-full w-full object-cover ${isHovering && videoReady ? "opacity-0" : "opacity-100"} transition-opacity duration-100`}
        />
      ) : (
        <div
          className={`absolute inset-0 flex h-full w-full items-center justify-center bg-zinc-800 ${isHovering && videoReady ? "opacity-0" : "opacity-100"} transition-opacity duration-100`}
        >
      ...
      <canvas
        ref={canvasRef}
        className={`absolute inset-0 h-full w-full object-cover ${isHovering && videoReady ? "opacity-100" : "opacity-0"} transition-opacity duration-100`}
      />
      ...
      {isHovering && videoReady && scrubTime && (
        <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 rounded bg-black/80 px-1.5 py-0.5 backdrop-blur-sm">
          <span className="font-mono text-[9px] tabular-nums text-white">
            {scrubTime}
          </span>
        </div>
      )}
