...
import type { TimelineClip, TimelineGap, Track } from "../../types/project";
...
interface KeyboardRefs {
  kbLayoutRef: React.MutableRefObject<KeyboardLayout>;
  ...
  gapGenerateModeRef: React.MutableRefObject<
    "text-to-video" | "image-to-video" | "text-to-image" | null
  >;
...
interface KeyboardSetters {
  ...
  setGapGenerateMode: React.Dispatch<
    React.SetStateAction<
      "text-to-video" | "image-to-video" | "text-to-image" | null
    >
  >;
  setSelectedGap: (v: TimelineGap | null) => void;
...
interface KeyboardContext {
  ...
  deleteGapRef: React.MutableRefObject<
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const {
        ...
        selectedClipIds: sel,
        totalDuration: td,
        selectedAssetIds: selAssets,
      } = refs.keyboardStateRef.current;

      const action: ActionId | null = resolveAction(
        refs.kbLayoutRef.current,
        e,
      );
      if (!action) return;
      ...
      switch (action) {
        // Tools
        case "tool.select":
          setters.setActiveTool("select");
          break;
        case "tool.blade":
          setters.setActiveTool("blade");
          break;
        case "tool.ripple":
          setters.setActiveTool("ripple");
          setters.setLastTrimTool("ripple");
          break;
        case "tool.roll":
          setters.setActiveTool("roll");
          setters.setLastTrimTool("roll");
          break;
        case "tool.slide":
          setters.setActiveTool("slide");
          setters.setLastTrimTool("slide");
          break;
        case "tool.slip":
          setters.setActiveTool("slip");
          setters.setLastTrimTool("slip");
          break;
        case "tool.trackForward":
          setters.setActiveTool("trackForward");
          break;
        ...
        case "transport.shuttleReverse":
          if (refs.activePanelRef.current === "source") {
            if (refs.sourceVideoRef.current) {
              refs.sourceVideoRef.current.pause();
              setters.setSourceIsPlaying(false);
              refs.sourceVideoRef.current.currentTime = Math.max(
                0,
                refs.sourceVideoRef.current.currentTime - FRAME_DURATION,
              );
            ...
            if (kHeldRef.current) {
              ...
              setters.setCurrentTime((prev) =>
                Math.max(0, prev - FRAME_DURATION),
              );
            } else {
              setters.setShuttleSpeed((prev) => {
                ...
                const nextIdx =
                  idx >= 0 ? Math.min(idx + 1, REVERSE_SPEEDS.length - 1) : 0;
          ...
          if (refs.activePanelRef.current === "source") {
            if (refs.sourceVideoRef.current) {
              if (kHeldRef.current) {
                refs.sourceVideoRef.current.pause();
                setters.setSourceIsPlaying(false);
                refs.sourceVideoRef.current.currentTime = Math.min(
                  refs.sourceVideoRef.current.duration || 0,
                  refs.sourceVideoRef.current.currentTime + FRAME_DURATION,
                );
                setters.setSourceTime(refs.sourceVideoRef.current.currentTime);
              } else {
            ...
            if (kHeldRef.current) {
              ...
              setters.setShuttleSpeed(0);
              setters.setCurrentTime((prev) =>
                Math.min(td, prev + FRAME_DURATION),
              );
            } else {
              setters.setShuttleSpeed((prev) => {
                if (prev < 0) return 1;
                ...
                const nextIdx =
                  idx >= 0 ? Math.min(idx + 1, FORWARD_SPEEDS.length - 1) : 0;
        ...
        case "transport.stepBackward":
          if (
            refs.activePanelRef.current === "source" &&
            refs.sourceVideoRef.current
          ) {
            refs.sourceVideoRef.current.currentTime = Math.max(
              0,
              refs.sourceVideoRef.current.currentTime - FRAME_DURATION,
        ...
        case "transport.jumpBackward":
          if (
            ...
            refs.sourceVideoRef.current.currentTime = Math.max(
              0,
              refs.sourceVideoRef.current.currentTime - 1,
            );
            setters.setSourceTime(refs.sourceVideoRef.current.currentTime);
          } else {
        ...
        case "transport.goToIn": {
          ...
          const target =
            inPoint ??
            (clips.length > 0 ? Math.min(...clips.map((c) => c.startTime)) : 0);
          setters.setIsPlaying(false);
          setters.setShuttleSpeed(0);
          setters.setCurrentTime(target);
          break;
        }
        case "transport.goToOut": {
          ...
          const target =
            outPoint ??
            (clipsOut.length > 0
              ? Math.max(...clipsOut.map((c) => c.startTime + c.duration))
              : tdOut);
          setters.setIsPlaying(false);
          setters.setShuttleSpeed(0);
          setters.setCurrentTime(target);
        ...
        case "edit.cut":
          ...
          break;
        case "edit.copy":
          refs.copyRef.current();
          break;
        case "edit.paste":
          refs.pasteRef.current();
          break;
        case "edit.selectAll":
        ...
        case "edit.deselect":
          if (refs.gapGenerateModeRef.current) {
            setters.setGapGenerateMode(null);
            setters.setSelectedGap(null);
          } else {
        ...
        case "edit.delete":
          if (sel.size > 0) {
            ...
            for (const id of sel) {
              ...
              if (clip && refs.tracksRef.current[clip.trackIndex]?.locked)
                continue;
              if (clip?.linkedClipIds)
                clip.linkedClipIds.forEach((lid) => deleteIds.add(lid));
            }
            setters.setClips((prev) =>
              prev.filter((cl) => !deleteIds.has(cl.id)),
            );
            setters.setSelectedClipIds(new Set());
          } else if (context.selectedGap) {
            refs.pushUndoRef.current();
            context.deleteGapRef.current(context.selectedGap);
            ...
            selAssets.forEach((id) =>
              context.deleteAsset(context.currentProjectId!, id),
            );
        ...
        case "edit.overwriteEdit":
          ...
          break;
        case "edit.matchFrame":
          refs.matchFrameRef.current();
          break;
        ...
        case "mark.setIn":
          if (refs.activePanelRef.current === "source") {
            ...
            setters.setSourceIn((prev) =>
              prev !== null && Math.abs(prev - st) < 0.01 ? null : st,
            );
          } else {
        ...
        case "timeline.zoomOut":
          ...
          setters.setZoom((prev) =>
            Math.max(refs.getMinZoomRef.current(), +(prev - 0.25).toFixed(2)),
          );
          break;
        ...
        case "nav.nextEdit": {
          ...
          for (const cl of refs.clipsRef.current) {
            ...
            editPts2.add(
              Math.round((cl.startTime + cl.duration) * 1000) / 1000,
            );
          }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- All deps are stable refs and setters; mount/unmount only
  }, []);
}
