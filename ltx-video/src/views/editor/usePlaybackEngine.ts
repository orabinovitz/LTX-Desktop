...
interface ExtendedHTMLVideoElement extends HTMLVideoElement {
  __pendingCanplay?: boolean;
  __syncOnLoad?: (() => void) | null;
}

interface ExtendedHTMLAudioElement extends HTMLAudioElement {
  __intendedSrc?: string;
...
export interface UsePlaybackEngineParams {
  ...
  crossDissolveState: CrossDissolveState | null;
...
export function usePlaybackEngine(params: UsePlaybackEngineParams) {
  ...
  useEffect(() => {
    ...
    const resolveClipSrcRef = (clip: TimelineClip): string => {
      ...
      if (clip.assetId) {
        const liveAsset = assetsRef.current.find((a) => a.id === clip.assetId);
        if (liveAsset) {
          if (
            liveAsset.takes &&
    ...
    const getNextVideoClip = (afterClip: TimelineClip): TimelineClip | null => {
      ...
      for (const c of all) {
        if (c.type === "audio" || c.type === "adjustment" || c.type === "text")
          continue;
    ...
    const getDissolveAtTime = (
      ...
      for (const clipA of all) {
        ...
        const progress = Math.max(
          0,
          Math.min(1, timeIntoDissolve / dissolveDuration),
        );
    ...
    const tick = (timestamp: number) => {
      ...
      if (dissolveInfo) {
        ...
        if (outSrc) {
          ...
          if (outVid) {
            ...
            if (container && !outVid.parentElement)
              container.appendChild(outVid);
              ...
              if (!isNaN(vd)) {
                ...
                const tt = outClip.reversed
                  ? Math.max(
                      ...
                      Math.min(
                        vd,
                        outClip.trimStart + usable - timeInClip * outClip.speed,
                      ),
                    )
                  : Math.max(
                      0,
                      Math.min(
                        vd,
                        outClip.trimStart + timeInClip * outClip.speed,
                      ),
                    );
                if (outClip.reversed) {
          ...
          if (inVid.duration && !isNaN(inVid.duration)) {
            ...
            const targetTime = clip.reversed
              ? Math.max(
                  ...
                  Math.min(
                    videoDuration,
                    clip.trimStart + usableMedia - timeInClip * clip.speed,
                  ),
                )
              : Math.max(
                  0,
            ...
            if (
              !isNaN(targetTime) &&
              Math.abs(inVid.currentTime - targetTime) > 0.04
            ) {
        ...
        if (dissolveInfo.incoming.asset?.type === "video") {
          ...
          if (inSrc && !pool.has(inSrc)) {
            ...
            v.muted = true;
            v.style.cssText =
              "position:absolute;inset:0;width:100%;height:100%;object-fit:contain;opacity:0;z-index:0;";
            v.src = inSrc;
            v.load();
            pool.set(inSrc, v);
            const container = document.getElementById("video-pool-container");
        ...
        if (clipSrc) {
          ...
          if (video) {
            const seekAndPlay = (v: HTMLVideoElement) => {
              ...
              if (!isNaN(videoDuration)) {
                const usableMedia =
                  videoDuration - syncClip.trimStart - syncClip.trimEnd;
                const targetTime = syncClip.reversed
                  ...
                  : Math.max(
                      ...
                      Math.min(
                        videoDuration,
                        syncClip.trimStart + timeInClip * syncClip.speed,
                      ),
                    );
                ...
                if (syncClip.reversed) {
                  ...
                  if (
                    !isNaN(targetTime) &&
                    Math.abs(v.currentTime - targetTime) > 0.04
                  ) {
                  ...
                  if (
                    !isNaN(targetTime) &&
                    Math.abs(v.currentTime - targetTime) > 0.3
                  ) {
                    if (typeof v.fastSeek === "function")
                      v.fastSeek(targetTime);
                    else v.currentTime = targetTime;
            ...
            if (video.readyState >= 2) {
              seekAndPlay(video);
            } else if (!(video as ExtendedHTMLVideoElement).__pendingCanplay) {
              // Video not decoded yet — seek & play as soon as it's ready (one listener only)
              (video as ExtendedHTMLVideoElement).__pendingCanplay = true;
              const onReady = () => {
                video.removeEventListener("canplay", onReady);
                (video as ExtendedHTMLVideoElement).__pendingCanplay = false;
                video.style.opacity = "1";
                video.style.zIndex = "1";
                seekAndPlay(video);
              };
          ...
          (
            previewVideoRef as React.MutableRefObject<HTMLVideoElement | null>
          ).current = video || null;
          ...
          if (nextClip && nextClip.id !== preSeekDoneRef.current) {
            const remainingInCurrent =
              syncClip.startTime + syncClip.duration - next;
            if (remainingInCurrent < 1.5 && remainingInCurrent > 0) {
              ...
              if (nextVideo && nextVideo.readyState >= 1) {
                ...
                if (!isNaN(nextTargetTime)) {
                  if (typeof nextVideo.fastSeek === "function")
                    nextVideo.fastSeek(nextTargetTime);
      ...
      //   4. Keep audio elements alive (just pause) so buffers are preserved.
      {
        ...
        for (const c of allClips) {
          ...
          if (
            c.type === "video" &&
            (!c.linkedClipIds ||
              !c.linkedClipIds.some((lid) =>
                allClips.some((ac) => ac.id === lid && ac.type === "audio"),
              ))
          )
            continue;
        ...
        for (const [id, el] of audioMap) {
          if (!activeAudioIds.has(id)) {
            ...
            (el as ExtendedHTMLAudioElement).__audioPlaying = false;
          }
        }
        ...
        for (const c of allClips) {
          ...
          if (!el) {
            ...
            el.src = url;
            (el as ExtendedHTMLAudioElement).__intendedSrc = url;
            el.preload = "auto";
          ...
          if (el.readyState >= 2) {
            if (!(el as ExtendedHTMLAudioElement).__audioPlaying || isNew) {
              ...
              if (!c.reversed) {
                el.play().catch(() => {});
                (el as ExtendedHTMLAudioElement).__audioPlaying = true;
                (el as ExtendedHTMLAudioElement).__lastPlayRetry = 0;
              }
            } else {
              // Already playing — minimal intervention
              if (el.playbackRate !== desiredRate)
                el.playbackRate = desiredRate;
              if (c.reversed) {
                if (!el.paused) el.pause();
              } else {
                ...
                if (el.paused) {
                  ...
                  if (now - lastRetry > 500) {
                    (el as ExtendedHTMLAudioElement).__lastPlayRetry = now;
                    el.play().catch(() => {});
                  }
                }
              }
            }
          } else if (!(el as ExtendedHTMLAudioElement).__awaitingCanplay) {
            (el as ExtendedHTMLAudioElement).__awaitingCanplay = true;
            const onCanPlay = () => {
              el!.removeEventListener("canplay", onCanPlay);
              (el as ExtendedHTMLAudioElement).__awaitingCanplay = false;
      ...
      const updateInterval = dissolveInfo
        ? DISSOLVE_STATE_UPDATE_INTERVAL
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- Refs and setState are stable; only reactive values listed
  }, [
    isPlaying,
    totalDuration,
    shuttleSpeed,
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- trackContainerRef is a stable ref
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- Refs are stable; only reactive values listed
  ...
  useEffect(() => {
    ...
    for (const src of timelineVideoSources) {
      if (!pool.has(src)) {
        ...
        video.style.cssText =
          "position:absolute;inset:0;width:100%;height:100%;object-fit:contain;opacity:0;z-index:0;pointer-events:none;";
        video.src = src;
        video.load();
        pool.set(src, video);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- videoPoolRef is a stable ref
  ...
  useEffect(() => {
      ...
      if (incoming.asset?.type === "video") {
        ...
        if (video) {
          ...
          if (
            ...
            !video.src.endsWith(incomingSrc)
          ) {
            video.src = incomingSrc;
            video.load();
          }
          ...
          const syncIncoming = () => {
            ...
            const usableMedia =
              videoDuration - incoming.trimStart - incoming.trimEnd;
            const targetTime = incoming.reversed
              ...
              : Math.max(
                  ...
                  Math.min(
                    videoDuration,
                    incoming.trimStart + timeInClip * incoming.speed,
                  ),
                );
          ...
          if (video.readyState >= 2) {
            ...
            video.addEventListener("loadeddata", () => syncIncoming(), {
              once: true,
            });
          }
        }
    ...
    const syncVideo = (forceSeek: boolean) => {
      ...
      if (syncClip.reversed) {
        ...
        if (
          !isNaN(targetTime) &&
          (forceSeek || Math.abs(video.currentTime - targetTime) > 0.04)
        ) {
        ...
        if (
          !isNaN(targetTime) &&
          (forceSeek || Math.abs(video.currentTime - targetTime) > 0.3)
        ) {
          if (forceSeek && Math.abs(video.currentTime - targetTime) < 0.001) {
            video.currentTime = targetTime + 0.001;
          }
    ...
    if (video.readyState >= 2) {
      ...
      (video as ExtendedHTMLVideoElement).__syncOnLoad = onLoaded;
    }

    return () => {
      const handler = (video as ExtendedHTMLVideoElement).__syncOnLoad;
      if (video && handler) {
        video.removeEventListener("loadeddata", handler);
        delete (video as ExtendedHTMLVideoElement).__syncOnLoad;
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps -- Refs (videoPoolRef, activePoolSrcRef, previewVideoRef) are stable
  }, [
    currentTime,
    isPlaying,
    activeClip,
  ...
  useEffect(() => {
    ...
    const allAudioClips = clips.filter((c) => {
      if (c.type === "adjustment" || c.type === "text" || c.type === "image")
        return false;
      ...
      if (
        ...
        (!c.linkedClipIds ||
          !c.linkedClipIds.some((lid) =>
            clips.some((ac) => ac.id === lid && ac.type === "audio"),
          ))
      )
        return false;
      return true;
    ...
    for (const clip of allAudioClips) {
      ...
      if (!el) {
        el = document.createElement("audio");
        el.src = clipUrl;
        (el as ExtendedHTMLAudioElement).__intendedSrc = clipUrl;
        el.preload = "auto";
      ...
      const liveAsset = clip.assetId
        ...
        : null;
      const assetDuration =
        liveAsset?.duration || clip.asset?.duration || clip.duration;
