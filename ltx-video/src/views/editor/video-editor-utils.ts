...
export function resolveOverlaps(
  ...
  for (const movedId of movedIds) {
    ...
    for (const c of result) {
      ...
      if (c.trackIndex !== moved.trackIndex) {
        next.push(c);
        continue;
      }
      ...
      if (cEnd <= movedStart || cStart >= movedEnd) {
        next.push(c);
        continue;
      }
...
export function loadLayout(): EditorLayout {
  try {
    ...
    if (stored) {
      ...
      return {
        leftPanelWidth: clampVal(
          parsed.leftPanelWidth ?? DEFAULT_LAYOUT.leftPanelWidth,
          LAYOUT_LIMITS.leftPanelWidth,
        ),
        rightPanelWidth: clampVal(
          parsed.rightPanelWidth ?? DEFAULT_LAYOUT.rightPanelWidth,
          LAYOUT_LIMITS.rightPanelWidth,
        ),
        timelineHeight: clampVal(
          parsed.timelineHeight ?? DEFAULT_LAYOUT.timelineHeight,
          LAYOUT_LIMITS.timelineHeight,
        ),
        assetsHeight: parsed.assetsHeight
          ? clampVal(parsed.assetsHeight, LAYOUT_LIMITS.assetsHeight)
          : 0,
      };
    }
  } catch {
...
export function getClipEffectStyles(
  ...
  if (cc.highlights !== 0)
    filters.push(`brightness(${1 + cc.highlights / 300})`);
  ...
  if (timeInClip !== undefined) {
    ...
    if (
      tIn &&
      tIn.type.startsWith("wipe-") &&
      tIn.duration > 0 &&
      timeInClip < tIn.duration
    ) {
  ...
  switch (fx.type) {
    ...
    case "glow": {
      ...
      if (intensity > 0) {
        filters.push(`brightness(${1 + intensity * 0.3})`);
        filters.push(
          `drop-shadow(0 0 ${radius * intensity}px rgba(255,255,255,${intensity * 0.4}))`,
        );
      }
      break;
    }
...
export function buildMaskImageValue(mask: EffectMask): string {
  ...
  const filterDef =
    featherDev > 0.5
      ? `<filter id="mf"><feGaussianBlur stdDeviation="${featherDev.toFixed(1)}"/></filter>`
      : "";
