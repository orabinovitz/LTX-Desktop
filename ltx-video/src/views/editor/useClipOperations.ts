...
interface UseClipOperationsParams {
  ...
  addAsset: (projectId: string, data: Omit<Asset, "id" | "createdAt">) => Asset;
  addTimeline: (projectId: string, name?: string) => Timeline;
  updateTimeline: (
    projectId: string,
    timelineId: string,
    data: Partial<Pick<Timeline, "tracks" | "clips" | "subtitles">>,
  ) => void;
...
export function useClipOperations(params: UseClipOperationsParams) {
  ...
  const addClipToTimeline = (
    ...
    const createVideoClip =
      (isVideoAsset || isImageAsset || isAdjustment) && videoPatched;
    ...
    const audioPatched =
      needsAudioClip &&
      tracks.some(
        (t) => t.kind === "audio" && !t.locked && t.sourcePatched !== false,
      );
    ...
    if (clipStartTime === undefined) {
      ...
      clipStartTime = trackClips.reduce(
    ...
    if (createVideoClip) {
      const newClip: TimelineClip = {
        ...
        ...(createAudioClip && audioTrackIndex >= 0
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (electronFilePath) {
        ...
        persistentUrl = normalized.startsWith("/")
          ? `file://${normalized}`
          : `file:///${normalized}`;
  ...
  const splitClipAtPlayhead = (
    ...
    for (const splitId of splittable) {
      ...
      newClips = newClips
        .map((c) => (c.id === splitId ? firstHalf : c))
        .concat(secondHalf);
      ...
      for (const linkedClip of linkedClips) {
        ...
        if (
          linkedSplitPoint <= 0.01 ||
          linkedSplitPoint >= linkedClip.duration - 0.01
        ) {
        ...
        const linkedFirstHalf: TimelineClip = {
          ...
          trimEnd:
            linkedClip.trimEnd + (linkedClip.duration - linkedSplitPoint),
      ...
      firstHalf.linkedClipIds = firstHalfLinkedIds.length
        ? firstHalfLinkedIds
        : undefined;
      secondHalf.linkedClipIds = secondHalfLinkedIds.length
        ? secondHalfLinkedIds
        : undefined;
  ...
  const removeClip = (clipId: string) => {
    ...
    if (clip?.linkedClipIds)
      clip.linkedClipIds.forEach((lid) => removeIds.add(lid));
  ...
  const addCrossDissolve = useCallback(
    (leftClipId: string, rightClipId: string) => {
    ...
    [clips, pushUndo, setClips, setHoveredCutPoint],
  );

  const removeCrossDissolve = useCallback(
    (leftClipId: string, rightClipId: string) => {
      pushUndo();
      setClips((prev) =>
        prev.map((c) => {
          if (c.id === leftClipId) {
            return {
              ...c,
              transitionOut: { type: "none" as TransitionType, duration: 0.5 },
            };
          }
          if (c.id === rightClipId) {
            return {
              ...c,
              transitionIn: { type: "none" as TransitionType, duration: 0.5 },
            };
          }
          return c;
        }),
    ...
    [pushUndo, setClips, setHoveredCutPoint],
  );

  const addTrack = (kind: "video" | "audio" = "video") => {
    const sameKindCount = tracks.filter(
      (t) => t.kind === kind && t.type !== "subtitle",
    ).length;
    const newTrack: Track = {
      id: `track-${Date.now()}`,
      name:
        kind === "audio" ? `A${sameKindCount + 1}` : `V${sameKindCount + 1}`,
      muted: false,
      locked: false,
      kind,
    };
  ...
  const deleteTrack = (idx: number) => {
    ...
    setClips((prev) =>
      prev
        ...
        .map((c) =>
          c.trackIndex > idx ? { ...c, trackIndex: c.trackIndex - 1 } : c,
        ),
    );
    ...
    setSubtitles((prev) =>
      prev
        ...
        .map((s) =>
          s.trackIndex > idx ? { ...s, trackIndex: s.trackIndex - 1 } : s,
        ),
    );
  ...
  const addTextClip = useCallback(
    (
      styleOverride?: Partial<TextOverlayStyle>,
      atTime?: number,
      trackIdx?: number,
    ) => {
      ...
      if (targetTrack === undefined) {
        ...
        targetTrack =
          videoTrackIndices.length > 0
            ? videoTrackIndices[videoTrackIndices.length - 1]
            : 0;
      }
  ...
  const handleImportTimeline = useCallback(
    async (parsed: ParsedTimeline) => {
      ...
      for (const ref of parsed.mediaRefs) {
        ...
        const resolution =
          ref.width && ref.height ? `${ref.width}x${ref.height}` : "Unknown";
      ...
      const totalTracks = Math.max(
        parsed.videoTrackCount + parsed.audioTrackCount,
        1,
      );
      ...
      for (let pcIdx = 0; pcIdx < parsed.clips.length; pcIdx++) {
        ...
        const clip: TimelineClip = {
          ...
          volume:
            pc.volume !== undefined ? Math.min(1, Math.max(0, pc.volume)) : 1,
      ...
      for (let pcIdx = 0; pcIdx < parsed.clips.length; pcIdx++) {
        ...
        if (audioClip && videoClip) {
          ...
          if (!audioClip.linkedClipIds.includes(videoClipId))
            audioClip.linkedClipIds.push(videoClipId);
          // Video clip collects all its audio clips
          if (!videoClip.linkedClipIds) videoClip.linkedClipIds = [];
          if (!videoClip.linkedClipIds.includes(audioClipId))
            videoClip.linkedClipIds.push(audioClipId);
        }
      }

      // Create a new timeline with these tracks and clips
      const newTimeline = addTimeline(
        currentProjectId,
        parsed.name || "Imported Timeline",
      );
