...
export function useTimelineDrag(params: UseTimelineDragParams) {
  ...
  const [resizingClip, setResizingClip] = useState<ResizingClipState | null>(
    null,
  );
  const [slipSlideClip, setSlipSlideClip] = useState<SlipSlideClipState | null>(
    null,
  );
  ...
  const scrubFromEvent = useCallback(
    ...
    [pixelsPerSecond, totalDuration, setCurrentTime, timelineRef],
  );
  ...
  const expandWithLinkedClips = useCallback(
  ...
  const handleClipMouseDown = (e: React.MouseEvent, clip: TimelineClip) => {
    ...
    if (activeTool === "slide") {
      ...
      const prevClip = sameTrack
        .filter((c) => c.startTime + c.duration <= clip.startTime + 0.05)
        .pop();
      const nextClip = sameTrack.find(
        (c) => c.startTime >= clip.startTime + clip.duration - 0.05,
      );
    ...
    if (activeTool === "trackForward") {
      const forwardClips = clips.filter((c) => {
        if (e.shiftKey) {
          ...
          return (
            c.trackIndex === clip.trackIndex && c.startTime >= clip.startTime
          );
        }
      });
      const forwardIds = expandWithLinkedClips(
      ...
      pushUndo();
      const originalPositions: Record<
        string,
        { startTime: number; trackIndex: number }
      > = {};
      clips
        .filter((c) => forwardIds.has(c.id))
        .forEach((c) => {
          originalPositions[c.id] = {
    ...
    if (
      ...
      if (e.shiftKey) {
        ...
        if (effectiveSelection.has(clip.id)) {
          ...
          if (!e.altKey && clip.linkedClipIds)
            clip.linkedClipIds.forEach((lid) => effectiveSelection.delete(lid));
        } else {
          effectiveSelection.add(clip.id);
          if (!e.altKey && clip.linkedClipIds)
            clip.linkedClipIds.forEach((lid) => {
      ...
      const expandedSelection = e.altKey
        ? effectiveSelection
        : expandWithLinkedClips(effectiveSelection);
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      let newStartTime =
        draggingClip.originalStartTime + deltaX / pixelsPerSecond;
      ...
      if (snapEnabled) {
        ...
        if (
          Math.abs(newStartTime + primaryClip.duration - currentTime) <
          snapThreshold
        ) {
      ...
      const resolveTrackForClip = (origTrackIndex: number): number => {
        ...
        const kindRows =
          clipKind === "audio" ? audioDisplayRows : videoDisplayRows;

        // Find this clip's position within its kind's ordered list
        const posInKind = kindRows.findIndex(
          (r) => r.realIndex === origTrackIndex,
        );
        if (posInKind === -1) return origTrackIndex;

        // Primary kind follows the drag direction; opposite kind mirrors (inverted delta)
        const effectiveDelta =
          clipKind === primaryKind ? rawDisplayDelta : -rawDisplayDelta;

        const newPosInKind = Math.max(
          0,
          Math.min(kindRows.length - 1, posInKind + effectiveDelta),
        );
    ...
    (e: MouseEvent) => {
          ...
          setClips((prev) =>
            prev.map((c) => {
              ...
              if (c.id === resizingClip.adjacentClipId) {
                const newStart =
                  (resizingClip.adjacentOrigStartTime ?? c.startTime) + dt;
                const newDur =
                  (resizingClip.adjacentOrigDuration ?? c.duration) - dt;
                const newTrimStart =
                  (resizingClip.adjacentOrigTrimStart ?? c.trimStart) +
                  dt * c.speed;
                return {
                  ...c,
                  startTime: newStart,
                  duration: Math.max(0.5, newDur),
                  trimStart: Math.max(0, newTrimStart),
                };
              }
              if (adjLinkedIds.has(c.id)) {
                const newStart =
                  (resizingClip.adjacentOrigStartTime ?? adjClip.startTime) +
                  dt;
                const newDur =
                  (resizingClip.adjacentOrigDuration ?? adjClip.duration) - dt;
            ...
            prev.map((c) => {
              ...
              if (linkedIds.has(c.id)) {
                ...
                return {
                  ...
                  startTime: newStart,
                  duration: newDur,
                  trimStart: Math.max(0, linkedTrimStart),
                };
              }
              if (c.id === resizingClip.adjacentClipId) {
      ...
      if (tool === "ripple") {
        ...
        if (resizingClip.edge === "left") {
          ...
          if (newStartTime < 0) {
            newDuration =
              resizingClip.originalDuration + resizingClip.originalStartTime;
            newStartTime = 0;
          }

          const newTrimStart =
            resizingClip.originalTrimStart +
            (newStartTime - resizingClip.originalStartTime);
          ...
          setClips((prev) =>
            prev.map((c) => {
              ...
              if (linkedIds.has(c.id)) {
                const linkedNewTrimStart =
                  c.trimStart + (newStartTime - resizingClip.originalStartTime);
                return {
                  ...c,
                  startTime: newStartTime,
                  duration: finalDuration,
                  trimStart: Math.max(0, linkedNewTrimStart),
                };
              }
              if (
                c.trackIndex === clip.trackIndex &&
                c.id !== clip.id &&
                c.startTime < resizingClip.originalStartTime
              ) {
                return {
                  ...c,
                  startTime: Math.max(0, c.startTime + rippleDelta),
                };
              }
              return c;
            }),
  ...
  const handleResizeStart = (
    ...
    if (activeTool === "roll") {
      ...
      if (edge === "right") {
        ...
        adjacentClip = clips.find(
          (c) =>
            c.id !== clip.id &&
            c.trackIndex === clip.trackIndex &&
            Math.abs(c.startTime - clipEnd) < 0.05,
        );
      } else {
    ...
    (e: MouseEvent) => {
      ...
      if (slipSlideClip.tool === "slip") {
        ...
        const minStart = slipSlideClip.prevClipId
          ? (clips.find((c) => c.id === slipSlideClip.prevClipId)?.startTime ??
            0)
          : 0;
        // Clamp: can't go past nextClip's end (or infinity)
        const nextEnd = slipSlideClip.nextClipId
          ? (slipSlideClip.nextOrigStartTime ?? 0) +
            (slipSlideClip.nextOrigDuration ?? 0)
          : Infinity;
        newStartTime = Math.max(
          minStart,
          Math.min(nextEnd - clip.duration, newStartTime),
        );
        ...
        setClips((prev) =>
          prev.map((c) => {
            ...
            if (slipSlideClip.prevClipId && c.id === slipSlideClip.prevClipId) {
              const newDur =
                (slipSlideClip.prevOrigDuration ?? c.duration) + actualDelta;
            ...
            if (slipSlideClip.nextClipId && c.id === slipSlideClip.nextClipId) {
              const newStart =
                (slipSlideClip.nextOrigStartTime ?? c.startTime) + actualDelta;
              const newDur =
                (slipSlideClip.nextOrigDuration ?? c.duration) - actualDelta;
              const newTrimStart =
                (slipSlideClip.nextOrigTrimStart ?? c.trimStart) +
                actualDelta * c.speed;
              return {
                ...c,
                startTime: newStart,
  ...
  const handleTrackDrop = (e: React.DragEvent, trackIndex: number) => {
    ...
    if (timelineData && trackContainerRef.current) {
      const droppedTimeline = JSON.parse(timelineData) as {
        id: string;
        name: string;
      };
      ...
      const earliestStart = sourceTimeline.clips.reduce(
        (min: number, c: TimelineClip) => Math.min(min, c.startTime),
        Infinity,
      );

      const newClips = sourceTimeline.clips.map((srcClip: TimelineClip) =>
        migrateClip({
          ...srcClip,
