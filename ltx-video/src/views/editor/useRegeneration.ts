...
export interface UseRegenerationParams {
  ...
  addTakeToAsset: (
    projectId: string,
    assetId: string,
    take: { url: string; path: string; createdAt: number },
  ) => void;
  deleteTakeFromAsset: (
    projectId: string,
    assetId: string,
...
export function useRegeneration(params: UseRegenerationParams) {
  ...
  const [icLoraSourceClipId, setIcLoraSourceClipId] = useState<string | null>(
    null,
  );

  // Error state for imported assets that can't auto-generate a prompt
  const [regenerationPreError, setRegenerationPreError] = useState<
    string | null
  >(null);
  ...
  const handleI2vGenerate = useCallback(async () => {
    ...
    const settings: GenerationSettings = {
      ...i2vSettings,
      duration: Math.min(
        Math.max(1, Math.round(clip.duration)),
        i2vSettings.model === "pro" ? 10 : 20,
      ),
    };
  ...
  const handleRegenerate = useCallback(
    async (assetId: string, clipId?: string) => {
      ...
      if (clipId) {
        setClips((prev) =>
          prev.map((c) =>
            c.id === clipId ? { ...c, isRegenerating: true } : c,
          ),
        );
      }
      ...
      if (!params) {
        try {
          ...
          const clipSrc = resolveClipSrc(
            clips.find((c) => c.id === clipId) ||
              ({ asset, assetId: asset.id } as unknown as TimelineClip),
          );
          let frameBase64 = "";
          ...
          if (frameBase64) {
            ...
            const resp = await fetch(`${backendUrl}/api/suggest-gap-prompt`, {
              ...
              body: JSON.stringify({
                ...
                mode:
                  asset.type === "image" ? "text-to-image" : "text-to-video",
                beforePrompt: "",
                afterPrompt: "",
            ...
            if (resp.ok) {
              ...
              if (data.suggested_prompt) {
                params = {
                  mode:
                    asset.type === "image" ? "text-to-image" : "image-to-video",
          ...
          console.warn(
            "Failed to auto-generate prompt for imported asset:",
            err,
          );
        }
      ...
      if (params.mode === "text-to-image") {
        ...
        const imagePath =
          params.mode === "image-to-video" && params.inputImageUrl
  ...
  const handleCancelRegeneration = useCallback(() => {
    ...
    setRegeneratingClipId(null);
    regenReset();
  }, [regenCancel, regenReset, regeneratingClipId, setClips]);
  ...
  const handleRetakeSubmit = useCallback(
    async (params: {
      ...
      try {
        ...
        if (response.ok && data.status === "complete" && data.video_path) {
          ...
          const retakeUrl = pathNormalized.startsWith("/")
            ? `file://${pathNormalized}`
            : `file:///${pathNormalized}`;
    ...
    [retakeClipId, clips, assets, currentProjectId, addTakeToAsset, setClips],
  );

  // IC-LoRA result handler
  const handleICLoraResult = useCallback(
    (result: { videoPath: string; sourceClipId: string | null }) => {
      if (!currentProjectId) return;

      const pathNormalized = result.videoPath.replace(/\\/g, "/");
      const videoUrl = pathNormalized.startsWith("/")
        ? `file://${pathNormalized}`
        : `file:///${pathNormalized}`;
      ...
      if (result.sourceClipId) {
        ...
        addAsset(currentProjectId, {
          ...
          resolution: "",
          takes: [
            { url: videoUrl, path: result.videoPath, createdAt: Date.now() },
          ],
          activeTakeIndex: 0,
        });
      }
    ...
    [currentProjectId, clips, assets, addTakeToAsset, addAsset, setClips],
  );

  // Handle take navigation on a clip (also updates linked audio/video clips)
  const handleClipTakeChange = useCallback(
    (clipId: string, direction: "prev" | "next") => {
      setClips((prev) => {
        ...
        const currentIdx =
          clip.takeIndex ?? asset.activeTakeIndex ?? asset.takes.length - 1;
  ...
  const handleDeleteTake = useCallback(
    (clipId: string) => {
      ...
      setClips((prev) =>
        prev.map((c) => {
          if (c.assetId !== asset.id) return c;
          const cIdx =
            c.takeIndex ?? asset.activeTakeIndex ?? asset.takes!.length - 1;
          if (cIdx === takeIdx) {
    ...
    [clips, assets, currentProjectId, deleteTakeFromAsset, setClips],
  );
