...
export function useGapGeneration({
  ...
  const [gapGenerateMode, setGapGenerateMode] = useState<
    ...
    tracks.forEach((track, trackIdx) => {
      ...
      if (trackClips[0].startTime > 0.05) {
        gaps.push({
          trackIndex: trackIdx,
          startTime: 0,
          endTime: trackClips[0].startTime,
        });
      }
      ...
      setSubtitles((prev) =>
        prev.map((s) => {
          if (s.startTime >= gap.endTime) {
            return {
              ...s,
              startTime: Math.max(0, s.startTime - gapDuration),
              endTime: Math.max(0.1, s.endTime - gapDuration),
            };
          }
          return s;
        }),
  ...
  const handleGapGenerate = useCallback(async () => {
    ...
    const settings: GenerationSettings = {
      ...gapSettings,
      duration: Math.min(
        Math.max(1, Math.round(gapDuration)),
        gapSettings.model === "pro" ? 10 : 20,
      ),
    };
    ...
    try {
      if (mode === "text-to-image" && gapImageFile) {
        ...
        if (gapImageFile) {
          const electronPath = (gapImageFile as File & { path?: string }).path;
          if (electronPath) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const asset = addAsset(currentProjectId, {
        ...
        url: finalUrl,
        prompt: gap.prompt,
        resolution: isImageResult
          ? gap.settings.imageResolution
          : gap.settings.videoResolution,
      ...
      const shouldCreateAudio =
        type === "video" && gap.applyAudio && gap.settings.audio;
      ...
      const newClip: TimelineClip = {
        ...
        opacity: 100,
        ...(shouldCreateAudio && audioTrackIndex >= 0
  ...
  const runSuggestion = useCallback(
    async (forceReplace: boolean = false) => {
      ...
      try {
          ...
          if (clipSrc) {
            if (clipBefore.asset?.type === "video") {
              const seekTime =
                ...
                clipBefore.duration * clipBefore.speed -
                0.1;
              framePromises.push(
        ...
        if (beforeFrame)
          setGapBeforeFrame(
            beforeFrame.startsWith("data:")
              ? beforeFrame
              : `data:image/jpeg;base64,${beforeFrame}`,
          );
        if (afterFrame)
          setGapAfterFrame(
            afterFrame.startsWith("data:")
              ? afterFrame
              : `data:image/jpeg;base64,${afterFrame}`,
          );
