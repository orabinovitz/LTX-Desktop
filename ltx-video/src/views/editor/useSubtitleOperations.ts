...
export function useSubtitleOperations({
  ...
  const [subtitleTrackStyleIdx, setSubtitleTrackStyleIdx] = useState<
    number | null
  >(null);
  ...
  const addSubtitleTrack = () => {
    ...
    setClips((prev) =>
      prev.map((c) => ({ ...c, trackIndex: c.trackIndex + 1 })),
    );
    setSubtitles((prev) =>
      prev.map((s) => ({ ...s, trackIndex: s.trackIndex + 1 })),
    );
  ...
  const handleImportSrt = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setSubtitles((prev) => [
        ...prev.filter((s) => s.trackIndex !== subtitleTrackIdx),
        ...newSubs,
      ]);
    };
