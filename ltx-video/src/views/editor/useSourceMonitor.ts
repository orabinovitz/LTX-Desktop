...
export function useSourceMonitor({
  ...
  const [activePanel, setActivePanel] = useState<"source" | "timeline">(
    "timeline",
  );
  ...
  const buildEditClips = useCallback(() => {
    ...
    const videoTrack = !isAudio
      ? trks.find(
          (t) => !t.locked && t.sourcePatched !== false && t.kind === "video",
        )
      : undefined;
    const audioTrack = trks.find(
      (t) => !t.locked && t.sourcePatched !== false && t.kind === "audio",
    );
  ...
  const handleInsertEdit = useCallback(() => {
    ...
    const { newClips, insertDuration, videoTrackIndex, audioTrackIndex, time } =
      result;
    setClips((prev) => {
      // Ripple clips on all targeted tracks
      const rippled = prev.map((c) => {
        const isTargetTrack =
          (videoTrackIndex >= 0 && c.trackIndex === videoTrackIndex) ||
  ...
  }, [pushUndo, buildEditClips, setClips]);
