...
export interface ProgramMonitorProps {
  ...
  previewPanRef: React.MutableRefObject<{
    dragging: boolean;
    startX: number;
    startY: number;
    startPanX: number;
...
export function ProgramMonitor({
  ...
  React.useEffect(() => {
    ...
    const poolVideo = document
      .getElementById("video-pool-container")
      ?.querySelector("video") as HTMLVideoElement | null;
    if (!poolVideo) return;
    for (const overlay of overlays) {
      const maskVideo = document.getElementById(
        `mask-video-${overlay.effectId}`,
      ) as HTMLVideoElement | null;
      if (
        maskVideo &&
        Math.abs(maskVideo.currentTime - poolVideo.currentTime) > 0.04
      ) {
  ...
  return (
    <div
      className={`flex flex-col ${showSourceMonitor ? "" : "flex-1"} min-h-0 min-w-0 ${showSourceMonitor && activePanel === "timeline" ? "ring-2 ring-inset ring-blue-500" : ""}`}
      style={
        showSourceMonitor
          ? { width: `${100 - sourceSplitPercent}%` }
          : undefined
      }
    ...
    >
      ...
      {showSourceMonitor && (
        <div className="flex h-7 flex-shrink-0 items-center border-b border-zinc-800 bg-zinc-900 px-3">
      ...
      <div
        ...
        className={`relative min-h-0 min-w-0 flex-1 overflow-hidden ${isFullscreen ? "bg-black" : ""}`}
        style={{
          backgroundColor: isFullscreen ? "#000" : "#333",
          ...(previewZoom !== "fit" ? { cursor: "grab" } : {}),
        }}
        onMouseDown={(e) => {
        ...
        onMouseMove={(e) => {
          ...
          setPreviewPan({
            x:
              previewPanRef.current.startPanX +
              (e.clientX - previewPanRef.current.startX),
            y:
              previewPanRef.current.startPanY +
              (e.clientY - previewPanRef.current.startY),
          });
        }}
        onMouseUp={() => {
        ...
        {clips.length === 0 ? (
          <div className="flex h-full w-full items-center justify-center">
            <div className="text-center">
              <div className="mx-auto mb-4 flex h-28 w-48 flex-col items-center justify-center rounded-lg border-2 border-dashed border-zinc-700">
                <Layers className="mb-2 h-8 w-8 text-zinc-600" />
                <p className="text-xs text-zinc-500">Drop clips here</p>
              </div>
            ...
            >
              {(() => {
                ...
                const dissolveOutOpacity = crossDissolveState
                  ? (1 - crossDissolveState.progress) *
                    ((crossDissolveState.outgoing.opacity ?? 100) / 100)
                  : undefined;
                return (
                  <>
                    ...
                    {compositingStack.map((lowerClip) => {
                      ...
                      const lowerStyles = getClipEffectStyles(
                        lowerClip,
                        lowerOffset,
                      );
                      const lowerSrc =
                        getClipUrl(lowerClip) ||
                        lowerClip.asset?.url ||
                        lowerClip.importedUrl ||
                        "";
                      if (
                        lowerClip.asset?.type === "image" ||
                        lowerClip.type === "image"
                      ) {
                        return (
                          <img
                      ...
                      const timeInClip = Math.max(
                        0,
                        currentTime - lowerClip.startTime,
                      );
                      return (
                        <video
                          key={`comp-${lowerClip.id}`}
                    ...
                    {activeClip &&
                      (() => {
                        const tInBg =
                          activeClip.transitionIn?.type !== "none"
                            ? getTransitionBgColor(activeClip.transitionIn.type)
                            : null;
                        const tOutBg =
                          activeClip.transitionOut?.type !== "none"
                            ? getTransitionBgColor(
                                activeClip.transitionOut.type,
                              )
                            : null;
                        const bg = tInBg || tOutBg;
                        if (!bg) return null;
                        const effectStyles = getClipEffectStyles(
                          activeClip,
                          clipPlaybackOffset,
                        );
                        const overlayOpacity =
                          effectStyles.opacity !== undefined
                            ? 1 - (effectStyles.opacity as number)
                            : 0;
                    ...
                    <div
                      ...
                      className={`pointer-events-none absolute inset-0 z-[2] h-full w-full ${!isPlaying && monitorClip?.asset?.type !== "video" ? "hidden" : ""}`}
                      style={
                        monitorClip?.asset?.type === "video"
                          ? {
                              ...getClipEffectStyles(
                                monitorClip,
                                clipPlaybackOffset,
                              ),
                              ...(dissolveOutOpacity !== undefined
                                ? { opacity: dissolveOutOpacity }
                                : {}),
                            }
                          : undefined
                      }
                    ...
                    {activeClip?.asset?.type === "image" && (
                      <img
                        ref={
                          previewImageRef as React.RefObject<HTMLImageElement>
                        }
                    ...
                    {crossDissolveState &&
                      (() => {
                        ...
                        const inOpacity =
                          progress * ((incoming.opacity ?? 100) / 100);
                        const inStyle = {
                          ...getClipEffectStyles(incoming, inOffset),
                          opacity: inOpacity,
                        };
                        const inSrc =
                          getClipUrl(incoming) || incoming.asset?.url || "";
                        if (incoming.asset?.type === "video") {
                          return (
                            <video
                              ref={
                                previewVideoRef as React.RefObject<HTMLVideoElement>
                              }
                    ...
                    {!monitorClip &&
                      (() => {
                        const audioAtPlayhead = clips.filter(
                          (c) =>
                            c.type === "audio" &&
                            ...
                            <AudioWaveform
                              audioClips={audioAtPlayhead.map((c) => ({
                                url:
                                  ...
                                  "",
                                name:
                                  c.asset?.path || c.importedName || "Audio",
              ...
              {activeAdjustmentEffects.map(
                ({
                  ...
                  const backdropFilter =
                    filterStyle.filter && filterStyle.filter !== "none"
                      ? String(filterStyle.filter)
                      : undefined;
                  return (
                          ...
                          prev.map((c) =>
                            c.id === tc.id && c.textStyle
                              ? {
                                  ...
                                  textStyle: {
                                    ...c.textStyle,
                                    positionX: Math.round(px * 10) / 10,
                                    positionY: Math.round(py * 10) / 10,
                                  },
                                }
                              : c,
                          ),
                  ...
                  >
                    <div
                      style={{
                        ...
                        padding:
                          ts.padding > 0 ? `${ts.padding * 0.04}vh` : undefined,
                        borderRadius:
                          ts.borderRadius > 0
                            ? `${ts.borderRadius}px`
                            : undefined,
              ...
              {activeSubtitles.length > 0 && (
                <div className="pointer-events-none absolute inset-0 z-[25] flex flex-col justify-end">
                  {activeSubtitles.map((sub) => {
                    ...
                    return (
                        ...
                        className={`flex w-full ${
                          style.position === "top"
                            ...
                            : style.position === "center"
                              ? "absolute inset-0 items-center justify-center self-center"
                              : "self-end"
                        }`}
                        style={
                          style.position !== "center"
                      ...
                      >
                        <span
                          className="mx-auto inline-block max-w-[90%] whitespace-pre-wrap rounded px-3 py-1.5 text-center leading-snug"
                          style={{
              ...
              {activeLetterbox &&
                (() => {
                  ...
                  if (targetRatio >= containerRatio) {
                    const barPct =
                      ((1 - containerRatio / targetRatio) / 2) * 100;
                    return barPct > 0 ? (
                      <>
                        <div
                          className="pointer-events-none absolute left-0 right-0 top-0 z-[18]"
                          style={{
                            height: `${barPct}%`,
                            backgroundColor: activeLetterbox.color,
                            opacity: activeLetterbox.opacity,
                          }}
                        />
                        <div
                          className="pointer-events-none absolute bottom-0 left-0 right-0 z-[18]"
                          style={{
                            height: `${barPct}%`,
                            backgroundColor: activeLetterbox.color,
                            opacity: activeLetterbox.opacity,
                    ...
                    const barPct =
                      ((1 - targetRatio / containerRatio) / 2) * 100;
                    return barPct > 0 ? (
                      <>
                        <div
                          className="pointer-events-none absolute bottom-0 left-0 top-0 z-[18]"
                          style={{
                            width: `${barPct}%`,
                            backgroundColor: activeLetterbox.color,
                            opacity: activeLetterbox.opacity,
                        ...
                        />
                        <div
                          className="pointer-events-none absolute bottom-0 right-0 top-0 z-[18]"
                          style={{
                            width: `${barPct}%`,
                            backgroundColor: activeLetterbox.color,
                            opacity: activeLetterbox.opacity,
      ...
      {clips.length > 0 && (
        <div className="relative flex-shrink-0 border-t border-zinc-800 bg-zinc-900 px-2 py-1">
          ...
          >
            ...
            {/* Progress fill */}
            <div
              className="absolute left-0 top-1/2 h-1 -translate-y-1/2 rounded-full bg-zinc-500"
              style={{
                width:
                  totalDuration > 0
            ...
            {(inPoint !== null || outPoint !== null) && (
              <div
                className="pointer-events-none absolute bottom-0 top-0 border-b-2 border-t-2 border-blue-400/70"
                style={{
                  left: `${((inPoint ?? 0) / totalDuration) * 100}%`,
                  width: `${(((outPoint ?? totalDuration) - (inPoint ?? 0)) / totalDuration) * 100}%`,
                }}
              >
                <div className="absolute inset-0 top-1/2 h-1 -translate-y-1/2 rounded-full bg-blue-400/30" />
              </div>
            )}
            ...
            {inPoint !== null && (
              <div
                ...
                style={{
                  left: `calc(${(inPoint / totalDuration) * 100}% - 6px)`,
                  width: 12,
                }}
                onMouseDown={(e) => {
                  e.stopPropagation();
              ...
              >
                <div className="pointer-events-none ml-auto flex h-full w-1 flex-col justify-between rounded-l-sm bg-blue-400 py-0.5">
                  <div className="h-0.5 w-2 rounded-r bg-blue-400" />
                  <div className="h-0.5 w-2 rounded-r bg-blue-400" />
                </div>
              </div>
            )}
            ...
            {outPoint !== null && (
              <div
                className="absolute bottom-0 top-0 z-10 flex cursor-ew-resize items-center"
                style={{
                  left: `${(outPoint / totalDuration) * 100}%`,
                  width: 12,
                }}
                onMouseDown={(e) => {
                  e.stopPropagation();
              ...
              >
                <div className="pointer-events-none flex h-full w-1 flex-col justify-between rounded-r-sm bg-blue-400 py-0.5">
                  <div className="-ml-1 h-0.5 w-2 rounded-l bg-blue-400" />
                  <div className="-ml-1 h-0.5 w-2 rounded-l bg-blue-400" />
                </div>
              </div>
            )}
            ...
            >
              <div className="absolute left-1/2 top-0 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-white" />
              <div className="absolute bottom-0 left-1/2 top-1.5 w-px -translate-x-1/2 bg-white" />
            </div>
          </div>
          {/* Timecode labels */}
          {(inPoint !== null || outPoint !== null) && (
            <div className="mt-0.5 flex h-3 items-center justify-between">
      ...
      <div className="flex h-8 flex-shrink-0 items-center gap-2 border-t border-zinc-800 bg-zinc-950 px-3">
        ...
        <span className="flex-shrink-0 select-none font-mono text-[12px] font-medium tabular-nums tracking-tight text-amber-400">
        ...
        <div className="relative flex-shrink-0">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setPreviewZoomOpen((prev) => !prev);
            }}
            className={`flex h-6 items-center gap-1 rounded border px-2 text-[11px] font-medium tabular-nums transition-colors ${
              previewZoomOpen
          ...
          {previewZoomOpen && (
            <div className="absolute bottom-full left-0 z-50 mb-1 min-w-[100px] rounded-lg border border-zinc-700 bg-zinc-900 py-1 shadow-2xl">
              {[
                ...
                >
                  ...
                  <span className={previewZoom === opt.value ? "" : "ml-5"}>
                    {opt.label}
                  </span>
                </button>
              ))}
        ...
        <div className="flex flex-1 items-center justify-center gap-0.5">
          {/* Set In */}
          <Tooltip
            content={`${inPoint !== null ? `In: ${formatTime(inPoint)} — ` : ""}Set In point (${getShortcutLabel(kbLayout, "mark.setIn")})`}
            side="top"
          >
            <Button
              variant="ghost"
          ...
          {/* Go to In */}
          <Tooltip
            content={`Go to In Point (${getShortcutLabel(kbLayout, "transport.goToIn")})`}
            side="top"
          >
            <Button
              variant="ghost"
              ...
              onClick={() => {
                const target =
                  inPoint ??
                  (clips.length > 0
                    ? Math.min(...clips.map((c) => c.startTime))
                    : 0);
                setIsPlaying(false);
                setShuttleSpeed(0);
          ...
          {/* Step Back */}
          <Tooltip
            content={`Step Back (${getShortcutLabel(kbLayout, "transport.stepBackward")})`}
            side="top"
          >
            <Button
              variant="ghost"
              ...
              onClick={() => {
                setShuttleSpeed(0);
                setIsPlaying(false);
                setCurrentTime((t) => Math.max(0, t - 1 / 24));
              }}
            >
          ...
          <Tooltip
            content={
              isPlaying
                ? `Pause (${getShortcutLabel(kbLayout, "transport.playPause")})`
                : `Play (${getShortcutLabel(kbLayout, "transport.playPause")})`
            }
            side="top"
          >
            <Button
          ...
          {/* Step Forward */}
          <Tooltip
            content={`Step Forward (${getShortcutLabel(kbLayout, "transport.stepForward")})`}
            side="top"
          >
            <Button
              variant="ghost"
              ...
              onClick={() => {
                setShuttleSpeed(0);
                setIsPlaying(false);
                setCurrentTime((t) => Math.min(totalDuration, t + 1 / 24));
              }}
            >
          ...
          {/* Go to Out */}
          <Tooltip
            content={`Go to Out Point (${getShortcutLabel(kbLayout, "transport.goToOut")})`}
            side="top"
          >
            <Button
              variant="ghost"
          ...
          {/* Set Out */}
          <Tooltip
            content={`${outPoint !== null ? `Out: ${formatTime(outPoint)} — ` : ""}Set Out point (${getShortcutLabel(kbLayout, "mark.setOut")})`}
            side="top"
          >
            <Button
              variant="ghost"
          ...
          <Tooltip content="Loop In/Out" side="top">
            <Button
              variant="ghost"
              size="icon"
              className={`h-6 w-6 ${playingInOut ? "bg-yellow-400/10 text-yellow-400" : "text-zinc-500"} ${inPoint === null || outPoint === null ? "cursor-not-allowed opacity-30" : ""}`}
        ...
        <div className="relative flex-shrink-0">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setPlaybackResOpen((prev) => !prev);
            }}
            className={`flex h-6 items-center gap-1 rounded border px-2 text-[11px] font-medium transition-colors ${
              playbackResolution === 1
                ? "border-zinc-700 bg-zinc-900 text-green-400 hover:border-zinc-600"
                : playbackResolution === 0.5
                  ? "border-zinc-700 bg-zinc-900 text-yellow-400 hover:border-zinc-600"
                  : "border-zinc-700 bg-zinc-900 text-orange-400 hover:border-zinc-600"
            }`}
          ...
          {playbackResOpen && (
            <div className="absolute bottom-full right-0 z-50 mb-1 min-w-[120px] rounded-lg border border-zinc-700 bg-zinc-900 py-1 shadow-2xl">
              {(
                [
                  {
                    label: "Full (1:1)",
                ...
                <button
                  key={opt.label}
                  onClick={() => {
                    setPlaybackResolution(opt.value);
                    setPlaybackResOpen(false);
                  }}
                  className={`flex w-full flex-col gap-0 px-3 py-1.5 text-left text-[11px] transition-colors ${
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={playbackResolution === opt.value ? "" : "ml-5"}
                    >
                      {opt.label}
                    </span>
                  </div>
        ...
        {/* Fullscreen */}
        <Tooltip
          content={`${isFullscreen ? "Exit fullscreen" : "Fullscreen"} (${getShortcutLabel(kbLayout, "view.fullscreen")})`}
          side="top"
        >
          <button
        ...
        <span className="flex-shrink-0 select-none text-right font-mono text-[12px] font-medium tabular-nums tracking-tight text-zinc-400">
