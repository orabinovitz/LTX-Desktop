...
import {
  ...
  ChevronLeft,
  ChevronRight,
  Paintbrush, // IC-LORA HIDDEN: removed Sparkles
  Video,
  Camera,
} from "lucide-react";
import type {
...
export interface ClipContextMenuProps {
  ...
  setClipContextMenu: React.Dispatch<
    React.SetStateAction<{ clipId: string; x: number; y: number } | null>
  >;
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-3 px-3 py-1.5 text-left transition-colors ${
        disabled ? "cursor-not-allowed opacity-40" : "hover:bg-zinc-700"
      } ${danger ? "text-red-400" : "text-zinc-300"}`}
    >
      <Icon
        className={`h-3.5 w-3.5 flex-shrink-0 ${iconClass || (danger ? "" : "text-zinc-500")}`}
      />
      <span className="flex-1 truncate">{label}</span>
      {badge && (
        <span
          className={`flex-shrink-0 text-[10px] font-medium ${badgeClass || "text-zinc-500"}`}
        >
          {badge}
        </span>
      )}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="select-none px-3 pb-0.5 pt-1.5 text-[9px] font-semibold uppercase tracking-widest text-zinc-500">
      {children}
    </div>
  );
}
...
function Divider() {
  return <div className="my-1 h-px bg-zinc-700" />;
}
...
export function ClipContextMenu({
  ...
  return (
    <div
      ...
      className="fixed z-[60] min-w-[220px] max-w-[280px] rounded-xl border border-zinc-700 bg-zinc-800 py-1.5 text-xs shadow-2xl"
        ...
        <>
          ...
          {TEXT_PRESETS.slice(0, 4).map((preset) => (
            <button
              ...
              className="flex w-full items-center gap-3 px-3 py-1.5 pl-9 text-left text-zinc-400 hover:bg-zinc-700"
            >
              <span className="flex-shrink-0 text-[10px] text-cyan-500/70">
                T
              </span>
          ...
          contextClip={(() => {
            ...
            if (contextClip.linkedClipIds?.length) {
              const primary = clips.find(
                (c) =>
                  contextClip.linkedClipIds!.includes(c.id) &&
  ...
  return (
    <>
      ...
      <MenuItem
        icon={RotateCcw}
        label="Reverse"
        badge={contextClip.reversed ? "ON" : undefined}
      ...
      <MenuItem
        ...
        badge={contextClip.muted ? "MUTED" : undefined}
      ...
      <MenuItem
        ...
        badge={contextClip.flipH ? "ON" : undefined}
      ...
      <MenuItem
        icon={FlipVertical2}
        label="Flip Vertical"
        badge={contextClip.flipV ? "ON" : undefined}
        badgeClass="text-cyan-400"
        onClick={() => {
            ...
            setClips((prev) =>
              prev.map((c) => {
                ...
                if (allLinked.has(c.id)) {
                  const remaining = (c.linkedClipIds || []).filter(
                    (lid) => lid !== contextClip.id,
                  );
                  return {
                    ...c,
            ...
            <MenuItem
              ...
              onClick={() => {
                ...
                setClips((prev) =>
                  prev.map((c) => {
                    if (c.id === contextClip.id)
                      return { ...c, linkedClipIds: candidateIds };
                    if (candidateIds.includes(c.id))
                      return { ...c, linkedClipIds: [contextClip.id] };
                    return c;
                  }),
                );
                close();
        ...
        <button
          onClick={() => {
            if (contextClip.assetId) {
              ...
              if (currentProjectId)
                updateAsset(currentProjectId, contextClip.assetId, {
        ...
        <>
          ...
          {liveAsset!.takes && liveAsset!.takes.length > 1 && (
            <div className="flex items-center gap-2 px-3 py-1">
              ...
              <button
                ...
                className="rounded p-0.5 text-zinc-400 hover:bg-zinc-600 hover:text-white"
              >
                <ChevronLeft className="h-3 w-3" />
              </button>
              <span className="min-w-[28px] text-center text-[10px] tabular-nums text-zinc-300">
              ...
              <button
                onClick={(e) => {
                  ...
                  if (
                    confirm(
                      `Delete take ${(contextClip.takeIndex ?? liveAsset!.activeTakeIndex ?? liveAsset!.takes!.length - 1) + 1}?`,
                    )
                  ) {
                    handleDeleteTake(contextClip.id);
                  }
                }}
                className="ml-auto rounded p-0.5 text-zinc-500 hover:bg-red-900/50 hover:text-red-400"
          ...
          {(isVideo || isImage) && (
            <div className="group/capture relative">
              <button className="flex w-full items-center gap-3 px-3 py-1.5 text-left text-zinc-300 transition-colors hover:bg-zinc-700">
                <Camera className="h-3.5 w-3.5 flex-shrink-0 text-amber-400" />
                <span className="flex-1 truncate">Use Frame As...</span>
                <ChevronRight className="h-3 w-3 text-zinc-500" />
              </button>
              <div className="absolute left-full top-0 z-[70] ml-0.5 hidden min-w-[200px] rounded-lg border border-zinc-700 bg-zinc-800 py-1 shadow-xl group-hover/capture:block">
                <MenuItem
                  icon={Paintbrush}
                  iconClass="text-emerald-400"
                  label="Edit Image in Gen Space"
                  onClick={() => {
                    onCaptureFrameForEdit(contextClip);
                    close();
                  }}
                />
                <MenuItem
        ...
        <>
            ...
            <MenuItem
              ...
              onClick={() => {
                ...
                if (asset) {
                  ...
                  setTimeout(() => {
                    const card = assetGridRef.current?.querySelector(
          ...
          {(() => {
            ...
            if (liveAsset.takes && liveAsset.takes.length > 0) {
              ...
              filePath =
                liveAsset.takes[
                  Math.max(0, Math.min(takeIdx, liveAsset.takes.length - 1))
                ].path;
            }
...
function MultiClipMenu({
  ...
  const batchUpdate = (updates: Partial<TimelineClip>) => {
    pushUndo();
    setClips((prev) =>
      prev.map((c) => (selectedClipIds.has(c.id) ? { ...c, ...updates } : c)),
    );
    close();
  };
  ...
  const hasVideoAndAudio =
    selectedClips.some((c) => c.type === "video") &&
    selectedClips.some((c) => c.type === "audio");
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-1 px-3 py-1">
        {[0.25, 0.5, 1, 1.5, 2, 4].map((speed) => (
          <button
            ...
            className="rounded bg-zinc-700 px-1.5 py-0.5 text-[10px] font-medium text-zinc-400 transition-colors hover:bg-zinc-600 hover:text-white"
          >
            {speed}x
          </button>
        ))}
      ...
      <MenuItem
        icon={allMuted ? VolumeX : Volume2}
        label={allMuted ? "Unmute All" : "Mute All"}
        badge={allMuted ? "ALL MUTED" : undefined}
        badgeClass="text-red-400"
        onClick={() => batchUpdate({ muted: !allMuted })}
      />
      <MenuItem
        icon={RotateCcw}
        label={allReversed ? "Un-reverse All" : "Reverse All"}
        badge={allReversed ? "ALL ON" : undefined}
        badgeClass="text-blue-400"
      ...
      <MenuItem
        icon={FlipHorizontal2}
        label={allFlipH ? "Un-flip All Horizontal" : "Flip All Horizontal"}
        badge={allFlipH ? "ALL ON" : undefined}
        badgeClass="text-cyan-400"
        onClick={() => batchUpdate({ flipH: !allFlipH })}
      />
      <MenuItem
        icon={FlipVertical2}
        label={allFlipV ? "Un-flip All Vertical" : "Flip All Vertical"}
        badge={allFlipV ? "ALL ON" : undefined}
        badgeClass="text-cyan-400"
        ...
        <button
          onClick={() => {
            ...
            const affectedAssetIds = new Set(
              selectedClips.map((c) => c.assetId).filter(Boolean) as string[],
            );
            ...
            setClips((prev) =>
              prev.map((c) => {
                if (
                  ...
                  (c.assetId && affectedAssetIds.has(c.assetId))
                ) {
            ...
            if (currentProjectId) {
              affectedAssetIds.forEach((id) =>
                updateAsset(currentProjectId!, id, { colorLabel: undefined }),
              );
            }
            close();
          }}
          className="flex h-4 w-4 items-center justify-center rounded-full border-2 border-zinc-600 transition-all hover:border-zinc-400"
          title="No label"
        >
          <X className="h-2 w-2 text-zinc-400" />
        </button>
